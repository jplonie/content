...
class MITMProxy:
    ...
    def clean_mock_file(self, playbook_id, path=None, thread_index=0, prints_manager=None):
        ...
        if problem_keys and needs_whitewashing:
            ...
            debug_opt = f' | sudo tee -a {log_file} 2>&1'
    ...
    def start(self, playbook_id, path=None, record=False, thread_index=0, prints_manager=None):
        ...
        if self.is_proxy_listening():
            prints_manager.add_print_job('proxy service is already running, stopping it', print_error, thread_index)
            self.ami.call(['sudo', 'systemctl', 'stop', 'mitmdump'])
        self.prepare_proxy_start(path, playbook_id, record, prints_manager, thread_index)
        # Start proxy server
        self._start_proxy_and_wait_until_its_up(prints_manager, thread_index, is_record=record)

    def _start_proxy_and_wait_until_its_up(self, prints_manager, thread_index: int, is_record: bool) -> None:
        """
    ...
    def get_mitmdump_service_status(self, prints_manager, thread_index: int) -> None:
        """
        ...
        try:
            ...
            prints_manager.add_print_job(f'mitmdump service status output:\n{output.decode()}', print_error,
                                         thread_index)
        except CalledProcessError as exc:
            prints_manager.add_print_job(f'mitmdump service status output:\n{exc.output.decode()}', print_error,
                                         thread_index)
    ...
    def prepare_proxy_start(self,
        ...
        Args:
            ...
            prints_manager: A prints manager instance
            thread_index: The index of the current thread
        """
        path = path or self.current_folder
    ...
    def stop(self, thread_index=0, prints_manager=None):
        ...
        if not self.is_proxy_listening():
            prints_manager.add_print_job('proxy service was already down.', print_error, thread_index)
        else:
            self.ami.call(['sudo', 'systemctl', 'stop', 'mitmdump'])
